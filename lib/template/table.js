...
exports.records = function (args, rows) {
    ...
    for (var i=0; i < rows.length; i++) {
        ...
        if (args.config.editview.allowHardDelete) {
			var remove = {
				key: record+'[remove]',
        ...
        for (var j=0; j < columns.length; j++) {
            ...
            if (args.slug !== args.config.slug) {
            	if (column.inlineview && column.inlineview.show === false) {
            		columns.splice(j, 1);
            		j--;
            		continue;
				}
            ...
            args.error = column.error ? (args.error || column.error) : args.error;
        }
