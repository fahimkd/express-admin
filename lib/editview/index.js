...
exports.getTypes = function (args, done) {
    ...
    async.eachSeries(['view', 'oneToOne', 'manyToOne'], function (type, done) {
		...
		if (type === 'oneToOne') {
			Object.keys(args.tables).forEach((tableName) => {
				if (args.tables[tableName].linkColumn) {
					result.view.tables[0].records[0].columns.forEach((col) => {
						if (col.name === args.tables[tableName].linkColumn) {
							if (col.value.constructor === Array) {
								args.id = col.value.filter(row => row.selected)[0];
								args.id = [args.id.__pk ? args.id.__pk : args.id];
							} else {
								args.id = [col.value];
							}
						}
					})
					args.tables[tableName] = args.tables[tableName].refColumn;
				}
			})
		}
