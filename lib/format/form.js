...
exports.value = function (column, value) {
    ...
    else if (column.control.readonly) {
    	if (column.value && column.value.constructor === Array) {
    		selected = column.value.filter (row => row.__pk == value)[0];
    		return selected ? selected.__text : null;
		} else {
			return value
		}
    }
